    ...
    , ui(new Ui::Notepad)
{
    ...
    connect(ui->actionGenerate_Statistical_File,SIGNAL(triggered()),this,SLOT(StatisticalFile()));
    ...
    connect(ui->actionGraph, SIGNAL(triggered()), this, SLOT(showGraph()));
...
void Notepad::StatisticalFile()
{
  fstream myFile;

  if (currentFile == ui->editor->toPlainText())
  {
      QMessageBox::warning(this, "Warning", "Unable to generate a statistical file that is empty");
      return;
  }
  else if(currentFile != ui->editor->toPlainText())
  {
      QString fileName = QFileDialog::getSaveFileName(this, "Save Statistical File")+ ".sta";
      QFile file(fileName);
      ...
      char* date_time = ctime(&currentDate);
      int uniqueCount;

      // generates a list of word classes
      QList<QString> insertedWord;
      QList<Word>countedWords;


      QMutableListIterator<Word>index(countedWords);
      QString text = ui->editor->toPlainText();
      QString totalWords = ui->editor->toPlainText();


      insertedWord = text.split("");
      insertedWord = text.split(QRegularExpression("[\r\n\t ;:,.!?]+"));
      insertedWord.removeAll("");

      while(!insertedWord.isEmpty())
       {
          QString unique = insertedWord.first();
          uniqueCount = insertedWord.count(unique);

          index.insert(Word(unique));
          index.value().setCount(uniqueCount);

          insertedWord.removeAll(unique);

       }

       std::sort(countedWords.end(),countedWords.begin());

       // split the editor text into a word list
       QStringList wordList = totalWords.split(QRegularExpression("[\r\n\t ;:,.!?]+"), Qt::SkipEmptyParts);


       fileStream << "Statistical Date for: " << fileName <<"\n";
       fileStream << "Generated Date: " << date_time << "\n";
       fileStream << "Total word count: " << wordList.count() << "\n";
       fileStream << "Unique word count:" << uniqueCount << "\n";
       fileStream << "Word Frequency: " << "\n";
       fileStream << "/n";

       for(const Word& str : countedWords)
           fileStream << str.getText() << ": " << str.getCount() << "\n";
  ...
  else if(!currentFile.isEmpty())
  {
     ...
     QString fileName = QFileDialog::getSaveFileName(this, "Save as") + "_statistics.sta";
     QFile file(fileName);
